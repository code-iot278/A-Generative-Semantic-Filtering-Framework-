# =========================================================
# Full Gaussian Filtering + Metrics Evaluation
# =========================================================
...
# -----------------------------
# Gaussian Filtering Function (Grayscale)
# -----------------------------
def gaussian_filtering(image_gray, sigma=1.5, alpha=0.6):
    """
    ...
    - alpha: blending factor between original and filtered image (0-1)
    ...
    # Apply Gaussian blur
    img_gaussian = cv2.GaussianBlur(img, (0, 0), sigmaX=sigma, sigmaY=sigma)

    # Blend original and filtered image
    filtered = cv2.addWeighted(img, 1 - alpha, img_gaussian, alpha, 0)
...
def epi_metric(orig_gray, proc_gray):
    # Edge Preservation Index (using Sobel)
...
output_main_folder = "/content/drive/MyDrive/Colab Notebooks/UC MercedLand for Earth Scanning/images_gaussian_filtered"
...
for img_path in image_paths:
    ...
    # Apply Gaussian filtering on L channel
    l_channel_filtered = gaussian_filtering(l_channel, sigma=1.5, alpha=0.6)

    # Merge channels and convert back to RGB
    lab_filtered = cv2.merge((l_channel_filtered, a_channel, b_channel))
    color_filtered = cv2.cvtColor(lab_filtered, cv2.COLOR_LAB2RGB)

    # -----------------------------
    # Compute metrics
    # -----------------------------
    psnr_val = psnr_metric(l_channel, l_channel_filtered)
    mse_val = mse_metric(l_channel, l_channel_filtered)
    ssim_val = ssim_metric(l_channel, l_channel_filtered)
    epi_val = epi_metric(l_channel, l_channel_filtered)
    cii_val = cii_metric(l_channel, l_channel_filtered)
    ...
    cv2.imwrite(output_path, cv2.cvtColor(color_filtered, cv2.COLOR_RGB2BGR))
